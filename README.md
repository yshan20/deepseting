...
целиком сохраняются в `~/.claude/backups/` — путь внутри повторяет путь внутри каталога настроек,
поэтому откат сводится к обратному копированию. Рядом с оригиналом копии не складываются намеренно:
копия внутри `skills/` подхватывается Claude Code как ещё один скилл с почти тем же описанием,
а копия внутри `rules/` засоряет каталог правил. Сам `backups/` Claude Code не читает.
...
Каталог `backups/` не чистится автоматически — удаляй устаревшие копии сам.
