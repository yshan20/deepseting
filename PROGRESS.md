...
- Шаг 6: резервные копии перенесены в `<config>/backups/` с сохранением относительного пути.
...
- Резервные копии складываются в `<config>/backups/`, а не рядом с оригиналом: рядом они
